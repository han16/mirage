...
##################################
num.gene=1000
#m=100  # same number of variants in all genes 
#num_variants=c(50,100,500)  # 3 gene groups, 50 variants in a gene in group 1, 100 in group 2, and 500 in group 3
num_variants=sample(50:500, 1000, replace = TRUE) # each gene has a random number of variants between 50, and 500
gene_group=3
N0=3000; N1=3000
delta=0.2
alpha0 <- 0.1
...
for (run in 1:max.run) {
  ...
  #  if (i < num.gene * 0.1) {  # vary number of variants of every gene in 3 different gene groups
  #    m <- num_variants[1]
  #  } else if (i <= num.gene * 0.9) {
  #    m <- num_variants[2]
  #  } else {
  #    m <- num_variants[3]
  #  }
    ...
    m=num_variants[i]
      ...
      if (ncol(data$geno)>0)
        for (j in 1:ncol(data$geno))  
        {
          ...
          EUiZij[orig.var.index]=numer/denom
  ...
  #  }
    for (k in 1:num.group)
    {
      sum_EUiZij=0; sum_actu.no.var=0
      for ( g in 1:num.gene)
      {
     #   if (g < num.gene * 0.1) {
    #      m <- num_variants[1]
    #    } else if (g <= num.gene * 0.9) {
    #      m <- num_variants[2]
    #    } else {
    #      m <- num_variants[3]
    #    } 
        ...
        from=split.ratio[k]*m+1; to=split.ratio[k+1]*m
        sum_EUiZij=sum_EUiZij+sum(EUiZij_list[[g]][(from:to)])
        sum_actu.no.var=sum_actu.no.var+actu.no.var[g,k]*EUi[g]
      }
      
      
      beta.k[iter,k]=sum_EUiZij/sum_actu.no.var  
    }  # end of  for (k in 1:num.group) 
...
#save(Gene.Risk.Status, MIRAGE.pvalue, MIRAGE.BF, Fisher.pvalue, SKATO.pvalue, CMC.pvalue, 
#     ASUM.pvalue, Fisher.separate.pvalue, ACAT.pvalue, file="C:\\Users\\hans\\OneDrive - Marquette University\\RareVariant\\simulation\\7methods_delta0.2_MAC_threshold0.gammamean335_5rep.RData")
